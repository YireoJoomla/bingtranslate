...
class Token
{
	 ...
	 * @param $params
	 */
	public function __construct($params, \Yireo\Translate\Session $session)
	{
		$this->params  = $params;
	...
	protected function setTokenInSession($token)
	{
		...
		$this->session->save($this->getSubscriptionKey() . '.token', $token);
		$this->session->save($this->getSubscriptionKey() . '.time', time());
	...
	protected function getTokenFromSession()
	{
		...
		$tokenTime    = (int) $this->session->get($this->getSubscriptionKey() . '.time');
		$sessionToken = $this->session->get($this->getSubscriptionKey() . '.token');
